...
//         //订单
           $P=M('order');
           $order=$P->order('id desc')->limit(12)->select();
...
//  		 echo('<script>location.href="/Home/index/login.html"</script>');
	...
	public function dologin()
    {
        ...
        {
            echo"<script>alert('输入的验证码有误，请重新输入！');location.href='/Home/index/login?u=".$username."&v=".$code."'</script>";
            exit;
        }
        else
        {
        ...
        if(!empty($username)){
            ...
            if(!empty($iofo))
            {
                ...
                redirect('/home/wo/wo.html');
                }
                else
                {
                    echo"<script>alert('用户密码有误，请重新输入');location.href='/Home/index/login?u=".$username."&v=".$code."'</script>";
                    die();
                }
            }
            else
            {
                echo"<script>alert('用户帐户有误，请重新输入');location.href='/Home/index/login?u=".$username."&v=".$code."'</script>";
                die();
            }
            exit;
        }else{

            echo"<script>alert('会员帐户为空，请重新输入');location.href='/Home/index/login?u=".$username."&v=".$code."'</script>";
            die();
        }
    }
...
//		$data1=
    ...
    public function registered()
    {
        ...
        $hhh=str_replace('Mc','',$data['username']);
        $hhh=intval($hhh)+1;
...
//          }
   ...
   //极差奖
    public function userall($username)
    {
        ...
        if($data[0]['total']>=30&&$data[0]['total']<100)
        {
            $price=10000*0.02;
        }elseif($data[0]['total']>=100&&$data[0]['total']<200)
        {
            $price=10000*0.03;
        }elseif($data[0]['total']>=200&&$data[0]['total']<500)
        {
            $price=10000*0.04;
        }elseif($data[0]['total']>=500&&$data[0]['total']<1000)
        {
            $price=10000*0.05;
        }elseif($data[0]['total']>=1000&&$data[0]['total']<2000)
        {
            $price=10000*0.06;
        }elseif($data[0]['total']>=2000&&$data[0]['total']<3000)
        {
            $price=10000*0.07;
        }elseif($data[0]['total']>=3000&&$data[0]['total']<5000)
        {
            $price=10000*0.08;
        }elseif($data[0]['total']>=5000&&$data[0]['total']<8000)
        {
            $price=10000*0.09;
        }elseif($data[0]['total']>=8000)
        {
            $price=10000*0.1;
   ...
   public function dojjtx()
   {
            ...
            $admin=M('user');
       		...
       		if($this->_checkPwd($password,$info['password'])){
                ...
                echo"<script>alert('用户密码有误，请重新输入');location.href='/Home/index/jjtx.html'</script>";
                 die();
            }
       		if($pay==0){
                $arr = array(
	...
	{
		if(!$_SESSION['id']){
			echo "<script>alert('登录后购车！');location.href='/Home/index/login.html';</script>";
		}
		$id=I('get.id');
		$P=M('product');
...
//			$pp = $room->where()->limit(0,5)->select();
        ...
        $pageurl="/home/index/shopcar.html?";
    ...
    {
    	echo"<script>alert('程序猿正在努力开发！');location.href='/Home/index/index.html';</script>";
    }
	...
	//忘记密码
	 ...
	 public function doszmm()
	 {
	 	...
	 	if(empty($user)){
	 		echo "<script>alert('您输入的会员帐号不存在！');location.href='/Home/index/szmm.html'</script>";
	 	}
	 	$pap = M('user')->where('paper="'.$paper.'"')->find();
	 	if(empty($pap)){
	 		echo "<script>alert('抱歉，身份证号码不正确！');location.href='/Home/index/szmm.html'</script>";
	 	}
	 	$update = array('password'=>$newpassword,'id'=>$user['id']);
	 	M('user')->save($update);
	 	echo "<script>alert('密码修改成功！');location.href='/Home/index/login.html'</script>";
	 }

...
use Common\Controller\DemoController;
class WoController extends DemoController{
    /**
     * 密码加密
     */
...
//		
   ...
   public function jdmx()
   {
   		...
   		if($id!=""){
	   		if($id==1){
	   			$hh=$m->where("gotegary=1 and userid='".session('id')."'")->order('id desc')->select();
	   		}elseif($id==2){
	   			$hh=$m->where("gotegary=2 and userid='".session('id')."'")->order('id desc')->select();
	   		}elseif($id==4){
	   			$hh=$m->where("gotegary=4 and userid='".session('id')."'")->order('id desc')->select();
	   		}
   		}else{
// 			$hh=$m->where("userid=".session('id'))->order('id desc')->select();
	   		$page = empty($_REQUEST['page'])?1:$_REQUEST['page'];
	        $pagesize =10;
	        $sqlw.=" userid='".session('id')."'";
	        $room = M('income');
	        $hh=$room->order(" id desc")->where($sqlw)->limit(($page-1)*$pagesize,$pagesize)->select();
   ...
   public function deljdmx()
   {
   		$id = I('get.id');
   		M('income')->where("id=".$id)->delete();
   		echo "<script>alert('记录删除成功!');location.href='/pc/wo/jdmx.html';</script>";
   }
   //个人中心
   ...
   {
    $zz = M('bycar')->where("userid=".session('id'))->find();
   	$this->assign('zz',$zz);
   ...
   {	
		...
		if($this->_checkPwd($paypassword,$info['paypassword'])){
	        ...
	        echo"<script>alert('用户密码有误，请重新输入');location.href='/pc/wo/recharge.html'</script>";
	         die();
	    }
      	if (!$data) {
          	echo"<script>alert('转账账户错误，请核查');location.href='/pc/wo/recharge.html'</script>";
    	}else{
      		if($zzlx==0){
      			echo "<script>alert('请选择账户类型！');location.href='/pc/wo/recharge.html'</script>";
      		}else
      		{
		        if($zzlx==1){
		          	if($name==session('username')){
		            	if ($zz==2){
		            		...
		            		if($recharge>$res['integral1']){
		                    	echo"<script>alert('转账失败,余额不足！');location.href='/pc/wo/recharge.html'</script>";
		                	}else{
				                ...
				                $arr = array(
			                  	...
			                  	M('income')->add($arr);
			                  	echo"<script>alert('转账成功！');location.href='/pc/index/index.html'</script>";
			                  	die;
		            		}
		            	}
	          		}else{
	          			echo "<script>alert('推广积分只能转到自身账户下的注册积分！');location.href='/pc/wo/recharge.html'</script>";
	          		}	              
    ...
    public function fkxq()
    {
    	header("Content-Type: text/html;charset=utf-8");
    	$id=I('get.id');
    	$ccc = M("message")->where("id=".$id)->find();
//  	$D = M('message');
//  	$update = array('status'=>1,'id'=>$id);
//  	M('message')->save($update);
//  	$ccc = $D->where('id='.$id )->select();
		$hhah = M('message')->where("status = 2 and id=".$id)->find();
    	$this->assign('ccc',$ccc);
    	$this->assign('hhah',$hhah);
    	$this->display();
    }
    ...
    public function dosqfk()
    {
    	...
    	$arr = array('title'=>$title,'content'=>$content,'userid'=>$_SESSION['id'],'createtime'=>time());
   		$data = $B->where("userid='".session('id')."'")->add($arr);
   		echo "<script>alert('已成功留言！');location.href='/pc/wo/znxj.html'</script>";
    }
...
//  	echo $_SESSION['id'];die;
		...
		if($boo['integral3'] >= 10000){
			...
			echo "<script>alert('激活成功');location.href='/pc/wo/wtjg.html'</script>";
			
			
		}else{
			echo "<script>alert('激活失败,余额不足，请进行充值');location.href='/pc/wo/tjlb.html'</script>";
		}
    }
  ...
  //网体结构
    public function wtjg()
    {
    	...
    	$id = I('get.id');
    	if($name != ""){
    		$us = M('user')->where("username='".$name."' and status=0")->find();
    		if(empty($us))
            {
...
//      
	...
	public function dosqbdzx()
	{
		...
		$username = M('user')->where("id=".session('id'))->find();
		$arr = array(
			'userid'=>session('id'),
		...
		M('bd')->add($arr);
		echo "<script>alert('申请成功，我们会尽快处理');location.href='/pc/index/index.html';</script>";
	}
	//报单列表
	public function bdlb()
	{
	   	header("Content-Type: text/html;charset=utf-8");
//		$jhm = M('user')->where("fwzx='".session('username')."'")->select();
//		$this->assign('jhm',$jhm);
...
//      $sqlw='1=1 ';
	...
	private function nexts($data=array(),$tusername='',$dep=0){
		static $list = array();
		foreach($data as $k=>$v){
			if($v['tusername']==$tusername){
				$v['dep'] = $dep;
				$list[] = $v;
				$this->nexts($data,$v['username'],$dep+1);
			}
		}
		return $list;
	}
    ...
    public function doxjb()
    {
      ...
      $rbk = $D->where("userid={$_SESSION['id']}")->save($update);
        ...
        echo"<script>alert('充值到报单积分成功！');location.href='/Admin/index/xjb.html'</script>";
      }
    }
    ...
    public function dogcjjtx()
    {
          ...
          if($this->_checkPwd($password,$info['paypassword'])){
                ...
                echo"<script>alert('用户密码有误，请重新输入');location.href='/pc/wo/gcjjtx.html'</script>";
                 die();
            }
          ...
          if($price>=2000){
               if($fq['integral4']<$price){
                     echo "<script>alert('剩余余额不足');location.href='/pc/wo/gcjjtx.html'</script>";
          }else{
             if($price%100==0){  
          if($pay==0){
            $jg = $price-($price*0.2);
            $sxf = $jg-$price;
             ...
             echo "<script>alert('提现最低2000起，每次最少100的倍数，例如(2100，2200)！');location.href='/pc/wo/gcjjtx.html'</script>";
          }

            }
          }else{
              echo"<script>alert('提现积分不能低于2000！');location.href='/pc/index/index.html'</script>";
            }
    }
            ...
            $fq = $cd->where("userid={$_SESSION['id']}")->find();
             $P=M('parameter');
          	...
          	if($this->_checkPwd($password,$info['paypassword'])){
                ...
                echo"<script>alert('用户密码有误，请重新输入');location.href='/pc/wo/tx.html'</script>";
                 die();
            }
            
            if($info['bank']==""){
	            echo "<script>alert('您未填写开户银行,请先补齐您的资料才可进行提现');location.href='/pc/wo/yhxx.html'</script>";
	        }else if($info['bankname']==""){
	            echo "<script>alert('您的开户姓名为空,请先补齐您的资料才可进行提现');location.href='/pc/wo/yhxx.html'</script>";
	        }else if($info['bankcode']==""){
	            echo "<script>alert('您的银行卡号为空,请先补齐您的资料才可进行提现');location.href='/pc/wo/yhxx.html'</script>";
	        }
            ...
            if($cotegary==1)
            {
	          	if($price>=2000){
	                if($fq['integral1']<$price){
	                   echo "<script>alert('剩余余额不足');location.href='/pc/wo/tx.html'</script>";
	                }else{
	                  if($price%100==0){
	                  $jg = $price-($price*$pr1);
	                  $sxf = $jg-$price;
	                ...
	                M('tx')->where("userid={$_SESSION['id']}")->add($arr);
	                //未扣之前
	                $yuer = M('integral')->where("userid={$_SESSION['id']}")->find();
	                //扣费
	                $kf = M('integral')->where("userid={$_SESSION['id']}")->find();
	                ...
	                $arr = array(
	                    ...
	                    'status'=>0,
	                    'gotegary'=>1,
	                    'total'=>$yuer['integral1'],
	                    );
	                M('income')->add($arr);
	                echo"<script>alert('提现申请成功！');location.href='/pc/index/index.html'</script>";
	            }else{
	              echo "<script>alert('提现最低2000起，每次最少100的倍数，例如(2100，2200)！');location.href='/pc/wo/tx.html'</script>";
	            }
			        }
			    }
		    }
    ...
    public function doxjbtx()
    {
          ...
          if($this->_checkPwd($password,$info['paypassword'])){
                ...
                echo"<script>alert('用户密码有误，请重新输入');location.href='/pc/wo/xjbtx.html'</script>";
                 
          }
          if($price>=2000){
                if($fq['integral1']<$price){
                   echo "<script>alert('剩余余额不足');location.href='/pc/wo/xjbtx.html'</script>";
                }else{
                   if($price%100==0){
                if($pay==0){
                  $jg = $price-($price*0.05);
              ...
              echo "<script>alert('提现最低2000起，每次最少100的倍数，例如(2100，2200)！');location.href='/pc/wo/xjbtx.html'</script>";
          }

        }
    }else{
       echo"<script>alert('提现积分不能低于2000！');location.href='/pc/wo/xjbtx.html'</script>";
    }
...
//购车基金转账
   ...
   {  
      ...
      if($this->_checkPwd($password,$info['paypassword'])){
            ...
            echo"<script>alert('用户密码有误，请重新输入');location.href='/pc/wo/gcjjzz.html'</script>";
             die();
        }
      if (!$data) {
          echo"<script>alert('转账账户错误，请核查');location.href='/pc/wo/gcjjzz.html'</script>";
      }else{
          if($data['tohelp']==1){
               if ($zz==1) {
          $D=M('integral');
          ...
          if($recharge>$res['integral4']){
             echo"<script>alert('转账失败,余额不足！');location.href='/pc/wo/gcjjzz.html'</script>";
          }else{
        ...
        echo "<script>alert('填写的用户必须是服务中心！');location.href='/pc/wo/gcjjzz.html'</script>";
      }
      ...
      $room=$D->field('id,path')->where('username="'.$username.'"')->find();
        ...
        $data=$D->field('id,paypassword,path')->where('id='.$_SESSION['id'])->find();
            ...
            if(!in_array($room['id'],$arr)&&!in_array($data['id'],$arr1))
             {
                     echo "<script>alert('只有相同队伍才能互转！');location.href='/pc/wo/recharge.html'</script>";//不在同一条线上
                     die(); 
             }
   }
   //积分记录
     ...
     public function jh()
     {
     	...
     	if($ac['tohelp']==1){
     		...
     		if($bq['integral2']>10000){
                ...
                //             //if($i==6)
                //             //{
                //                 //$this->userout($pdata['id'],$arr[3]);
                //                 //$D->where('id='.$pdata['id'])->setinc('rounds',1);//轮数加１
                //             //}
			    ...
			    $arr = array(
		    		'userid'=>$_SESSION['id'],
                    'username'=>$_SESSION['username'],
		    		'title'=>'注册'.$jh['username'].'会员，消耗注册积分',
		    		'price'=>10000,
		    		'createtime'=>time(),
		    		'status'=>3,
		    		'gotegary'=>2,
                ...
                $eq = M('integral')->where('userid='.$pdata['id'])->find();
		        	...
		        	$arra = array(
		                    ...
		                    'title'=>'推荐'.$jh['username'].'会员，奖励推广积分',
		                    'price'=>2000,
		                    'createtime'=>$createtime,
		                    'status'=>1,
		                    'gotegary'=>1
				    ...
				    $arr = array(
				    		'userid'=>$_SESSION['id'],
		                    'username'=>$_SESSION['username'],
				    		'title'=>'激活'.$jh['username'].'会员，奖励推广积分',
				    		'price'=>500,
				    		'createtime'=>$createtime,
				    		'status'=>1,
				    		'gotegary'=>1
				    ...
				    //极差奖
				    $this->userall($jh['username'],$pdata['username'],0,0,0);
		     	echo "<script>alert('账号激活成功！');location.href='/pc/wo/zhjh.html'</script>";
     		}else{
     			echo "<script>alert('报单积分低于10000无法激活！');location.href='/pc/wo/tjlb.html'</script>";
     		}
     		
     	}else{
     		echo "<script>alert('暂时只能由报单中心方可激活！');location.href='/pc/wo/tjlb.html'</script>";
     	}
    ...
    public function swtjg()
    {
     ...
     public function zhjh()
     {
     	...
     	if($zi['tohelp']==1){
     		...
     		echo "<script>alert('暂时只能由报单中心方可激活!');location.href='/pc/index/index.html'</script>";
     	}
     }
     ...
     public function dojhzh()
     {
        ...
        if(empty($tdata))
     	...
     	if($idata['integral2']<$pdata[18]['number'])
     	{
        ...
        $update = array('status'=>1,'id'=>$id);
		     	...
		     	$reb=$idata['integral2']-$pdata[18]['number'];
				$updata = array('integral2'=>$reb);
			    M('integral')->where("userid={$_SESSION['id']}")->save($updata);
			   
                 $this->Pay_history($_SESSION['id'],'注册'.$username.'会员，消耗注册积分',$pdata[18]['number'],$createtime,0,2);


                //推荐人奖励２０００
                $eq = M('integral')->where('userid='.$troom['id'])->find();
	    	        $bc = $eq['integral1']+$pdata[24]['number'];
		        	$ge = array('integral1'=>$bc);
		        	$I->where("userid=".$troom['id'])->save($ge);
		        	$this->Pay_history($troom['id'],'推荐'.$username.'会员，奖励推广积分',$pdata[24]['number'],$createtime,1,1);
                    //服务中心500
		            $bbq = M('integral')->where("userid={$_SESSION['id']}")->find();
				    $ccq = $bbq['integral1']+$pdata[21]['number'];
		            $updata2 = array('integral1'=>$ccq);
				    $bb = $I->where("userid={$_SESSION['id']}")->save($updata2);
				   
				    $this->Pay_history($_SESSION['id'],'激活'.$username.'会员，奖励推广积分',$pdata[21]['number'],$createtime,1,1);
				    
				    //极差奖
				    //$this->userall($pdata['username']);
				    $this->userall($username,$room['tusername'],0,0,0);//报单中心
				    //判断是否出局
     ...
     public function dogcsq()
     {
     	...
     	if($idata['integral4']<12000)
     	{
	     	...
	     	M('bycar')->add($arr);
	     	echo "<script>alert('提交申请成功，我们会尽快为您审核!');location.href='/pc/index/index.html'</script>";
     	}
     }
     
     //报单删除
     public function delbduser()
     {
     	
     	$id = I('get.id');
		M('user')->where('id='.$id)->delete();
     	echo "<script>alert('删除成功!');location.href='/pc/index/index.html'</script>";
     }

							...
							<table class="table">
								<tr>
									...
									<?php if(is_array($room)): $i = 0; $__LIST__ = $room;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
											...
											<td>
												<div>
													<?php if($vo["status"] == 0 ): ?><a class="caozuo_btn">未激活</a>
													<?php else: ?>
														<a class="caozuo_btn ck_btn">已激活</a><?php endif; ?>
												</div>
											</td>
											<td>
												<?php if($vo["status"] == 0 ): ?><a href="/pc/wo/delbduser?id=<?php echo ($vo["id"]); ?>" onclick="return firm();">删除</a><?php endif; ?>
											</td>
...
<div>
    ...
    <nav>
        <ul class="pagination pagination-sm pull-right">
            ...
            <?php if($count < 5): $__FOR_START_505208797__=0;$__FOR_END_505208797__=$count;for($i=$__FOR_START_505208797__;$i < $__FOR_END_505208797__;$i+=1){ if($page == $i+1): ?><li class="active"><a ><?php echo ($i+1); ?></a></li>
                        <?php else: ?>
                        <li ><a href="<?php echo ($pageurl); ?>page=<?php echo ($i+1); echo ($search); ?>"><?php echo ($i+1); ?></a></li><?php endif; } ?>
                <?php else: ?>
                <?php if($page <= 2): $__FOR_START_1400006790__=1;$__FOR_END_1400006790__=6;for($i=$__FOR_START_1400006790__;$i < $__FOR_END_1400006790__;$i+=1){ if(($i) == $page): ?><li class="active"><a href="<?php echo ($pageurl); ?>page=<?php echo ($page); echo ($search); ?>" ><?php echo ($page); ?></a></li>
                            <?php else: ?>
                            <li ><a href="<?php echo ($pageurl); ?>page=<?php echo ($i); echo ($search); ?>"><?php echo ($i); ?></a></li><?php endif; } ?>
                    <?php elseif($count-$page < 5): ?>
                    <?php $__FOR_START_651729281__=$count-4;$__FOR_END_651729281__=$count+1;for($i=$__FOR_START_651729281__;$i < $__FOR_END_651729281__;$i+=1){ if(($page) == $i): ?><li class="active"><a href="<?php echo ($pageurl); ?>page=<?php echo ($i); echo ($search); ?>" ><?php echo ($i); ?></a></li>
                            <?php else: ?>
                            <li ><a href="<?php echo ($pageurl); ?>page=<?php echo ($i); echo ($search); ?>"><?php echo ($i); ?></a></li><?php endif; } ?>
                    <?php else: ?>
                    <?php $__FOR_START_1294060798__=$page-2;$__FOR_END_1294060798__=$page+3;for($i=$__FOR_START_1294060798__;$i < $__FOR_END_1294060798__;$i+=1){ if(($page) == $i): ?><li  class="active"><a href="<?php echo ($pageurl); ?>page=<?php echo ($i); echo ($search); ?>"><?php echo ($i); ?></a></li>
                            <?php else: ?>

    ...
    if(confirm('请确认提交排单？')) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data == 1) {
                    alert('得到帮助订单提交成功！');
                    location.href = '/user/lgethelp.html';
                }
                if (data == 2) {
                    alert('支付密码不正确，请重新输入！');
                    $('#paypassword').focus();
                ...
                //     $('#yzje').focus();
                //     $('#button').prop('disabled',false);
                //     location.href='/user/paidan.html';
                //
                // }
                if (data == 4) {
                    alert('提供帮助金额每次不能超过30000！');
                    $('#yzje').focus();
                    $('#button').prop('disabled', false);
